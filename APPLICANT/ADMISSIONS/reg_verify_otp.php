...
try {
    ...
    $hash = password_hash($signupData['password'] ?: $pass, PASSWORD_DEFAULT);
    ...
    if ($hasRole) {
        if ($hasFullName) {
            ...
            $stmt->execute([$session_email, $fullName, $hash, $roleValue]);
        } else {
        ...
        if ($roleId) {
            if ($hasFullName) {
                ...
                $stmt->execute([$session_email, $fullName, $hash, $roleId]);
            } else {
            ...
            if ($hasFullName) {
                ...
                $stmt->execute([$session_email, $fullName, $hash]);
            } else {
