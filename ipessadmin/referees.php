...
async function bulkVerifySelectedReferees() {
    ...
    try {
        const res = await fetch('api/admin/referees.php', {
            method: 'POST',
            body: formData
        });
...
async function contactApplicant(appId) {
    ...
    const res = await fetch('api/admin/referees.php', { method: 'POST', body: formData });
    const data = await res.json();
    if (!data.success) {
...
async function contactAllPendingReferees() {
    ...
    try {
        const listRes = await fetch('api/admin/referees.php?action=pending_contact_list');
        ...
        for (let i = 0; i < total; i++) {
            ...
            try {
                const itemRes = await fetch('api/admin/referees.php', { method: 'POST', body: formData });
