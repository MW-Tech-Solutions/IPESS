...
async function loadAssignments() {
    ...
    const url = `api/reviewer/assignments.php?action=list&faculty_id=${faculty}&department_id=${department}&programme_id=${programme}&course_id=${course}`;
    const res = await fetch(url);
    const data = await res.json();
    if (!data.success) {
...
async function submitFeedback() {
    ...
    const res = await fetch('api/reviewer/assignments.php', { method: 'POST', body: formData });
    const data = await res.json();
    if (!data.success) {
