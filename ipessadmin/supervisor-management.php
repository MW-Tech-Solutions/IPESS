...
<section class="panel">
    <div class="panel-header">
        <div>
            <h3 class="panel-title"><?php echo htmlspecialchars($deptName); ?> Supervisors</h3>
...
async function sendSupervisorNotification() {
    ...
    const res = await fetch('api/dept-admin/supervisors.php', { method: 'POST', body: formData });
    const data = await res.json();
    if (!data.success) {
...
async function balanceWorkload() {
    const res = await fetch('api/dept-admin/supervisors.php', { method: 'POST', body: new URLSearchParams({ action: 'balance' }) });
    const data = await res.json();
    if (!data.success) {
