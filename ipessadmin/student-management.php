...
<section class="panel">
    <div class="panel-header">
        <div>
            <h3 class="panel-title"><?php echo htmlspecialchars($deptName); ?> Students</h3>
...
async function loadSupervisorsForSelect() {
    ...
    if (!select) return;
    const res = await fetch('api/dept-admin/supervisors.php?action=list');
...
async function loadStudents() {
    const res = await fetch('api/dept-admin/students.php?action=list');
    const data = await res.json();
    if (!data.success) {
...
async function changeSupervisor(studentId) {
    ...
    const res = await fetch('api/dept-admin/students.php', { method: 'POST', body: formData });
    const data = await res.json();
    if (!data.success) {
...
async function bulkSendMessage() {
    ...
    for (const id of ids) {
        ...
        formData.append('student_id', id);
        formData.append('message', message);
        await fetch('api/dept-admin/students.php', { method: 'POST', body: formData });
    }
