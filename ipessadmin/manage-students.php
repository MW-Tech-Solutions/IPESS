        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <ul class="nav nav-tabs mb-3" id="studentManageTabs" role="tablist">
                    ...
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" data-bs-toggle="tab" data-bs-target="#student-contact-referee-tab" type="button"><i class="fas fa-envelope me-1"></i>Contact Referee</button>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" data-bs-toggle="tab" data-bs-target="#student-add-referee-tab" type="button"><i class="fas fa-user-plus me-1"></i>Add Referee</button>
                    </li>
                    <li class="nav-item" role="presentation">
                ...
                <div class="tab-content">
                    ...
                    <!-- Contact Referee Tab -->
                    <div class="tab-pane fade" id="student-contact-referee-tab">
                        <div class="mb-3">
                            <h6 class="fw-bold">Contact Referees</h6>
                            <p class="text-muted small">Send a verification request email to one or all referees. The referee will receive a link to fill in their assessment form.</p>
                        </div>
                        <div id="contactRefereeList">
                            <div class="text-muted py-3 text-center">Loading referees...</div>
                        </div>
                        <div class="mt-3 d-flex gap-2">
                            <button type="button" class="btn btn-primary" id="btnContactAllReferees">
                                <i class="fas fa-paper-plane me-1"></i> Contact All Referees
                            </button>
                        </div>
                        <div id="contactRefereeStatus" class="mt-2"></div>
                    </div>

                    <!-- Add Referee Tab -->
                    <div class="tab-pane fade" id="student-add-referee-tab">
                        <div class="mb-3">
                            <h6 class="fw-bold">Add New Referee</h6>
                            <p class="text-muted small">Add a new referee to this applicant's profile. After saving, you can contact them from the Contact Referee tab.</p>
                        </div>
                        <form id="addRefereeForm" class="row g-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" class="form-control" name="title" placeholder="e.g. Prof., Dr., Mr.">
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-12 d-flex justify-content-end gap-2">
                                <button type="submit" class="btn btn-success">
                                    <i class="fas fa-plus me-1"></i> Add Referee
                                </button>
                            </div>
                        </form>
                        <div id="addRefereeStatus" class="mt-2"></div>
                    </div>
...
function getManageStudentModal() {
    if (!manageStudentModalEl || typeof bootstrap === 'undefined') {
        console.error('Bootstrap is not available. Make sure bootstrap.bundle.min.js is loaded in the <head>.');
        return null;
    }
...
function setHiddenIds(studentUserId, applicationId) {
    ...
    const addRefForm = document.getElementById('addRefereeForm');
    if (addRefForm) {
        const s = addRefForm.querySelector('[name="student_user_id"]');
        const a = addRefForm.querySelector('[name="application_id"]');
        if (s) s.value = studentUserId;
        if (a) a.value = applicationId;
    }
}

async function loadStudentProfile(studentUserId) {
    const url = `api/super-admin/manage-students.php?action=fetch&student_user_id=${encodeURIComponent(studentUserId)}`;
    ...
    document.querySelectorAll('.manage-student-btn').forEach((btn) => {
        btn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (!modal) {
                    throw new Error('Bootstrap modal not available. Please refresh the page.');
                }
                modal.show();
            } catch (error) {
...
async function submitStudentForm(form) {
    ...
    const response = await fetch('api/super-admin/manage-students.php', {
        method: 'POST',
        body: payload,
...
function renderContactRefereeList(refs, appId, userId) {
    ...
    if (!container) return;
    if (_currentReferees.length === 0) {
        container.innerHTML = '<div class="text-muted py-3 text-center">No referees found for this student.</div>';
        return;
    }
    container.innerHTML = _currentReferees.map((ref, idx) => `
        <div class="card mb-2 border-secondary">
            <div class="card-body d-flex justify-content-between align-items-center py-2">
                <div>
                    <div class="fw-semibold">${idx + 1}. ${ref.full_name || 'Unnamed'}</div>
                    <div class="small text-muted">${ref.email || 'No email'} | ${ref.phone || 'No phone'} | ${ref.organization || ''}</div>
                </div>
                <button type="button" class="btn btn-outline-primary btn-sm contact-one-referee-btn" data-referee-id="${ref.referee_id}">
    ...
    container.querySelectorAll('.contact-one-referee-btn').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (status) status.innerHTML = `<div class="alert alert-danger py-2">${e.message}</div>`;
            } finally {
                btn.disabled = false;
                btn.innerHTML = '<i class="fas fa-paper-plane me-1"></i> Contact';
            }
        });
    });
}

const btnContactAll = document.getElementById('btnContactAllReferees');
if (btnContactAll) {
    btnContactAll.addEventListener('click', async () => {
        ...
        btnContactAll.textContent = 'Sending...';
        try {
            const fd = new FormData();
            ...
            btnContactAll.disabled = false;
            btnContactAll.innerHTML = '<i class="fas fa-paper-plane me-1"></i> Contact All Referees';
        }
    });
}

// ============================================================
// Add Referee Tab Logic
// ============================================================
const addRefereeForm = document.getElementById('addRefereeForm');
if (addRefereeForm) {
    addRefereeForm.addEventListener('submit', async (event) => {
        event.preventDefault();
        const status = document.getElementById('addRefereeStatus');
        const submitBtn = addRefereeForm.querySelector('[type="submit"]');
        submitBtn.disabled = true;
        submitBtn.textContent = 'Saving...';
        try {
            const fd = new FormData(addRefereeForm);
            const resp = await fetch('api/super-admin/manage-students.php', { method: 'POST', body: fd, credentials: 'same-origin' });
            const d = await resp.json();
            if (status) status.innerHTML = `<div class="alert alert-${d.success ? 'success' : 'danger'} py-2">${d.message}</div>`;
            if (d.success) {
                addRefereeForm.reset();
                // Keep hidden IDs
                const sid = biodataForm.querySelector('[name="student_user_id"]').value;
                const aid = biodataForm.querySelector('[name="application_id"]').value;
                addRefereeForm.querySelector('[name="student_user_id"]').value = sid;
                addRefereeForm.querySelector('[name="application_id"]').value = aid;
                // Reload the student profile to refresh referee lists
                if (sid) await loadStudentProfile(sid);
            }
        } catch (e) {
            if (status) status.innerHTML = `<div class="alert alert-danger py-2">${e.message}</div>`;
        } finally {
            submitBtn.disabled = false;
            submitBtn.innerHTML = '<i class="fas fa-plus me-1"></i> Add Referee';
        }
    });
}
</script>
