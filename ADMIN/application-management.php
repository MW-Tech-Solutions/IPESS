...
// Base JOIN logic shared by both main query and count query to ensure filters work identically
$joinSql = " FROM applications a 
             ...
             LEFT JOIN courses c ON pc.course = c.course_id
             LEFT JOIN study_modes sm ON pc.mode_of_study = sm.mode_id"; 
             // Note: Ensure your DB column is 'mode_of_study' or change to 'mode' if step_9 implies 'mode'
...
$page = isset($_GET['page']) && is_numeric($_GET['page']) ? (int)$_GET['page'] : 1;
$limit = 10; // Adjusted limit for better visibility
$offset = ($page - 1) * $limit;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    viewMoreBtn.addEventListener('click', function () {
        ...
        fetch(`load-more-applications.php?page=${page}&${filters}`)
            .then(response => response.text())
            .then(data => {
                if (data.trim() !== '') {
                    activityList.insertAdjacentHTML('beforeend', data);
                } else {
                    viewMoreBtn.style.display = 'none';
                }
            })
            .catch(error => console.error('Error fetching more applications:', error));
    });
});
document.addEventListener('DOMContentLoaded', function () {
