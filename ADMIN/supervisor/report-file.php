...
$stmt = $pdo->prepare("SELECT report_name, format, file_path FROM admin_reports WHERE report_id = ? AND report_type LIKE 'Supervisor:%' LIMIT 1");
$stmt->execute([$id]);
...
if ($extension === 'pdf' && $mode === 'view' && !isset($_GET['raw'])) {
    ...
    $title = htmlspecialchars((string) $report['report_name'], ENT_QUOTES, 'UTF-8');
    ?>
<!DOCTYPE html>
<html lang="en">
<head>
