...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($failed_attempts >= $max_attempts && $first_attempt) {
        ...
        $stmt = $pdo->prepare("SELECT user_id, password_hash, account_status FROM applicant_accounts WHERE email = ?");
        $stmt->execute([$email]);
        $user = $stmt->fetch();
        ...
        if ($user && password_verify($password, $user['password_hash'])) {
            ...
            elseif ($user['account_status'] === 'Locked') {
                ...
                $updateStmt = $pdo->prepare("UPDATE applicant_accounts SET last_login = NOW() WHERE user_id = ?");
                $updateStmt->execute([$user['user_id']]);
                
                require 'helpers/load_application_data.php';
...
<head>
    <meta charset="UTF-8">
    <title>Applicant Login - JOSTUM</title>
    ...
    <style>
        ...
        body {
            background: linear-gradient(rgba(0, 0, 0, 0.2), rgba(0, 0, 0, 0.2)), 
                        url('./images/jostumgate.jpeg') no-repeat center center fixed;
...
<main class="login-card">
    <header>
        ...
        <h1 class="login-title">Applicant Login</h1>
    </header>
